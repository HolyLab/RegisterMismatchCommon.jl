...
function mismatch_apertures(::Type{T}, fixed::AbstractArray, moving::AbstractArray, gridsize::DimsLike, maxshift::DimsLike; kwargs...) where {T}
    ...
    return mismatch_apertures(T, fixed, moving, aperture_centers, aperture_width, maxshift; kwargs...)
end
...
function aperture_range(center, width)
    ...
    return ntuple(d -> leftedge(center[d], width[d]):rightedge(center[d], width[d]), length(center))
end
...
function default_aperture_width(img, gridsize::DimsLike, overlap::DimsLike = zeros(Int, sdims(img)))
    ...
    gflag = [gridsize...] .> 1
    return tuple((([map(d -> size(img, d), sc)...] - gflag) ./ gsz1 + 2 * [overlap...] .* gflag)...)
end
